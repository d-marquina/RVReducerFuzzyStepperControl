 ...
 * Constructor
 */
AS5048A::AS5048A(int8_t arg_cs, bool arg_debug /*=false*/)
	: _cs(arg_cs), spi(&SPI), errorFlag(false), ocfFlag(false), position(0), debug(arg_debug)
{
}
 ...
 *	Constructor with custom SPI, default pins
	*/
AS5048A::AS5048A(uint8_t spi_bus, int8_t arg_cs, bool arg_debug)
{
	this->_cs = arg_cs;
	this->errorFlag = false;
	this->ocfFlag = false;
	this->position = 0;
	this->debug = arg_debug;

	// Initialize custom SPI
	this->spi = new SPIClass(spi_bus);
}

/**
 *	Constructor with custom SPI, custom pins
	*/
AS5048A::AS5048A(uint8_t spi_bus, int8_t sck, int8_t miso, int8_t mosi, int8_t ss, bool arg_debug)
{
	this->errorFlag = false;
	this->ocfFlag = false;
	this->position = 0;
...
void AS5048A::beginCustomPins()
{
	...
	//SPI has an internal SPI-device counter, it is possible to call "begin()" from different devices
	this->spi->begin(this->_sck, this->_miso, this->_mosi, this->_cs);
}
...
uint16_t AS5048A::read(uint16_t registerAddress)
{
	...
	digitalWrite(this->_cs, LOW);
	uint16_t response = this->spi->transfer16(0x00);
	digitalWrite(this->_cs, HIGH);
